...
/**
 * 从生活护理等表中点击一个cell，变跳到这个页面
 *需要传入的参数有
 * 这个activity中的数据不需要要从网络上加载，加载数据是fragment自己的事情
 */
public class DetailQuestionActivity extends BaseActivity implements EvaInterActctionListnter{
	...
	protected void onDestroy() {
		...
		String key = tableType.getTableName()+CrossWiresFragment.STATE_SUFF_CROSS_LIST;
	...
	private void initFragmentToShow() {
		DetialQuestionFragment fragment = 
				DetialQuestionFragment.newInstance(crossList.get(currentPosition),tableType);
		
		getFragmentManager()
		.beginTransaction().replace(R.id.fl_evalution_main_container,
				fragment).commit();
	...
	/**
	 * 设置提交成功，做3件事
	 * 1.在记录已提交状态的表的 集合中加上这张表
	 * 2.在记录已提交的cross 的集合中添加此cross
	 * 3.跳到下一行
	 */
	private void setSumitSuccess(){
		...
		//表示哪些item被提交过了
		@SuppressWarnings("unchecked")
		HashSet<Integer> crossSet = (HashSet<Integer>) EvalutionState.stateMap.get(key);
		crossSet.add(crossList.get(currentPosition).getQuestionCrosswiseId());
		
		//表示这个表已经被提交过了。我们的策略是，只要有一个item被提交过了，这个表就算是提交了
		@SuppressWarnings("unchecked")
		HashSet<Integer> tableSet = (HashSet<Integer>) 
		EvalutionState.stateMap.get(EvalutionState.Keys.TABLE_SUBMIT_LIST);
	...
	private void nextCross(){
		...
		currentPosition++;
		initFragmentToShow();
		tvTitile.setText(crossList.get(currentPosition).getQuestionCrosswiseName());
	}
...
//		HttpUtils.requestPost(baseDataUrl,param.toParams(), new HttpUtils.ResultCallBack() {
//			@Override
...
//						List<QuestionPortrait> portraits = JSON.parseArray(data, QuestionPortrait.class);
