...
public class EditTimeActivity extends BaseActivity 
	...
	protected void getData() {
		...
		if(!TextUtils.isEmpty(serviceDate)){
			...
			LOG.e(TAG, "[getData] the extra "+EXTRA_SERVICE_DATE_STR+" didnt been pass to this activity");
		}
//		long timeMillon = getIntent().getLongExtra(EXTRA_SERVICE_DATE, 0);
//		if(timeMillon!=0){
//			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd", Locale.CHINA);
//			String dateStr = format.format(new Date(timeMillon));
//			getEditableTime(); 
//			LOG.i(TAG, "[getData]"+dateStr);
//		}
	}
	...
	protected void initViewConent() {
		...
		adapter = new CommonAdapter<TimeDuration>(this, data, R.layout.item_blue_gray_background_check) {
			@Override
			protected void convert(ViewHolder holder, TimeDuration item) {
				int position = holder.getPosition();
				String format = "%02d:00 - %02d:00";
				CheckBox chk = (CheckBox)holder.getView(R.id.chk_item_checkbox);
			...
			protected void initCreatedHolder(ViewHolder holder) {
				...
				CheckBox chk = (CheckBox)holder.getView(R.id.chk_item_checkbox);
				chk.setOnCheckedChangeListener(EditTimeActivity.this);
			}
		};
	...
	private void submitData(){
		if(selectedList.isEmpty()){
			AppContext.showToast("至少选择一个时间段");
			return ;
		}
		showWatingDailog();
		...
		BuisNetUtils.requestStr(url, param, new BuisNetUtils.ResponceCallBack() {
			...
			public void onSuccess(ResponseContent content) {
				...
				intent.putExtra(EditTimeActivity.EXTRA_IS_EDITE_SUCCESS, true);
