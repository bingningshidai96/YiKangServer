...
public class BusinessState {
	/**
	 * 是否添加了老人，如果是，下次在回到这个页面时，将重新请求数据
	 */
	...
	public static class SenoirState{
		...
		public static int currSeniorId; //当前老年人
		/**
		 *一次评估的评估状态，
		 *这个类的存在周期应该只在EvaluationActivity的生命周期之内，
		 *当EvaluationActivity销毁时，为防止内存泄漏，会调用{@link #clearAllState()}}此类中的状态会全部销毁
		 */
		public static class EvalutionState{
			...
			/**
			 * 用于添加其他数据，尤其注意用完后remove掉，避免内存泄漏。
			 * 禁止向着这里添加View相关的东西，因为容易造成内存泄漏。
			 */
