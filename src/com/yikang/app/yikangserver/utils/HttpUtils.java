...
public class HttpUtils {
	 ...
	 * 上传数据
	 */ 
	public static void uploadDate(String url,List<Object> params,ResultCallBack callBack){}
	
	public static void uploadFile(String url,Map<String,Object> param,ResultCallBack callBack){
		if(!checkNetWorkIsOk(AppContext.getAppContext())){
			callBack.postResult(null);
			return ;
		}
		...
		for (String key : keySet) {
			...
			if(object instanceof File){
				FileBody fileBody = new FileBody((File) param.get(key));
				//builder.addBinaryBody(key, (File) param.get(key));
				LOG.i(TAG, "[uploadFile]"+key+param.get(key));
				builder.addPart(key, fileBody);
			}else if(object instanceof String){
				StringBody stringBody = new StringBody((String) object, ContentType.TEXT_PLAIN);
				builder.addPart(key, stringBody);
				LOG.i(TAG, "[uploadFile]"+key+":"+(String) object);
			}else{
				throw new IllegalArgumentException("传入的map参数中只能包含File或者String对象");
			}
		}
		HttpStrAsycTask task = new HttpStrAsycTask(callBack);
	 ...
	 *
	 */
    private static class HttpStrAsycTask  extends AsyncTask<HttpUriRequest, Integer, String>{
