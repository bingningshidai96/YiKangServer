...
public class FreeTimeCalendarActivty extends BaseActivity implements OnItemClickListener{
	...
	protected void getData() {
		...
		BuisNetUtils.requestStr(url, param, new BuisNetUtils.ResponceCallBack() {
			...
			public void onSuccess(ResponseContent content) {
				...
				List<ServiceScheduleData> dataList = JSON.parseArray(result, 
						ServiceScheduleData.class);
	...
	protected void initViewConent() {
		...
		adapter = new CommonAdapter<ServiceScheduleData>(this,
				data,R.layout.item_servier_data_textview) {
			@Override
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			intent.putExtra(EditTimeActivity.EXTRA_SERVICE_DATE, schedule.serviceDate);
			intent.putExtra(EditTimeActivity.EXTRA_SERVICE_DATE_STR, schedule.serviceDayStr);
			startActivityForResult(intent, REQUEST_CODE_EDITTIME);
//		if(position==2){ /
//			ServiceScheduleData schedule = data.get(position);
//			updataPosition = position;
//			Intent intent = new Intent(this, EditTimeActivity.class);
//			intent.putExtra(EditTimeActivity.EXTRA_SERVICE_DATA, schedule.serviceDate);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		if(requestCode==REQUEST_CODE_EDITTIME&&intent!=null){
			boolean isSuccess = intent.getBooleanExtra(
					EditTimeActivity.EXTRA_IS_EDITE_SUCCESS, false);
			if(isSuccess){
