 ...
 * 长谷川评估表的适配器
 */
public class ChangGuChuangAdapter extends CommonChoiseAdapter<Question> implements OnDropDownItemClickListener{
	...
	protected void fillContent(ViewHolder holder, Question question) {
		...
		if(answerMap.containsKey(position)){
			...
			edpAnswers.setText(String.valueOf(getAnswerValue(position, selectAnswer)));
		}
	}
	...
	private void setEdpSelectedStyle(TextSpinner edtSpinner){
		...
		if(icArrowSelected ==null){
			...
			icArrowSelected.setBounds(new Rect(0, 0, icArrowSelected.getIntrinsicWidth(), icArrowSelected.getIntrinsicHeight()));
		}
	...
	private void setEdpUnselectedStyle(TextSpinner edtSpinner){
		...
		if(icArrowUnSelected ==null){ 
			...
			icArrowUnSelected.setBounds(
					new Rect(0, 0, icArrowUnSelected.getIntrinsicWidth(), 
							icArrowUnSelected.getIntrinsicHeight()));
		}
	...
	public void restoreAnswer(List<Question> questions) {
		for (Question question : questions) {
			...
			if(qPosition==-1||ans==null||ans.size()==0) continue ;
