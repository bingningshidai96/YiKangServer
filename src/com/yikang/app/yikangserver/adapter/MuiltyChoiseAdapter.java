...
public class MuiltyChoiseAdapter extends CommonChoiseAdapter<QuestionPortrait> implements
	...
	public MuiltyChoiseAdapter(Context context, List<QuestionPortrait> mDatas) {
		super(context, mDatas,  R.layout.item_question_common_multi, R.layout.item_answer_checkbox);
	...
	protected void fillContent(ViewHolder holder, QuestionPortrait item) {
		...
		tvQuestionText.setText((position+1)+". "+datas.get(position).getQuestionPortraitName());
	...
	public void restoreAnswer(List<QuestionPortrait> questions) {
		for (QuestionPortrait question : questions) {
			...
			for (int q=0;q<datas.size();q++) {
				if(datas.get(q).getQuestionPortraitId() 
						== question.getQuestionPortraitId()){
					qPosition = q;
