 ...
 *
 */
public class FallRiskAdapter extends CommonChoiseAdapter<CrossWirses> implements OnDropDownItemClickListener{
	...
	protected void fillContent(ViewHolder holder, CrossWirses item) {
		...
		List<QuestionPortrait> portraits = datas.get(position)
				.getQuestions();
	 ...
	 * @param portrait
	 */
	private void setPotrait(ViewGroup lyPortait,QuestionPortrait portrait,int crossPosition,int portraPosition){
		PortailHodler portailHodler = (PortailHodler) lyPortait
				.getTag();
		...
		portailHodler.tvPortailName.setText(portrait
				.getQuestionPortraitName());
	...
	private void setEdpStyle(TextSpinner edpAnswers,int crossPosition,int porPosition){
	...
	private void setEdpSelectedStyle(TextSpinner edtSpinner){
		...
		if(icArrowSelected ==null){
			icArrowSelected = context.getResources().
					getDrawable(R.drawable.ic_arrow_pull_down_black);
			icArrowSelected.setBounds(new Rect(0, 0, icArrowSelected.getIntrinsicWidth(), 
					icArrowSelected.getIntrinsicHeight()));
		}
	...
	private void setEdpUnselectedStyle(TextSpinner edtSpinner){
		...
		if(icArrowUnSelected ==null){ 
			icArrowUnSelected = context.getResources().
					getDrawable(R.drawable.ic_arrow_pull_down_white);
			icArrowUnSelected.setBounds(new Rect(0, 0, icArrowUnSelected.getIntrinsicWidth(), icArrowUnSelected.getIntrinsicHeight()));
		}
	...
	public void onItemClickListern(TextSpinner spinner, int position) {
		...
		if(icArrow ==null){
			icArrow = context.getResources().
					getDrawable(R.drawable.ic_arrow_pull_down_black);
			icArrow.setBounds(new Rect(0, 0, icArrow.getIntrinsicWidth(), icArrow.getIntrinsicHeight()));
		}
		...
		int weightVal = datas.get(crossPosition).getQuestions().get(porPosition).getWeightVal();
	...
	@SuppressLint("UseSparseArrays") 
	private void putAnswer(int crossPosition,int porPosition,int answerPosition){
	 ...
	 * @return
	 */
	private float getAnserValue(int crossPosition,int porPosition,int answerPosition){
		return datas.get(crossPosition).getQuestions().get(porPosition).getAnswers().get(answerPosition).getAnswerVal();
	}
	...
	public void restoreAnswer(List<CrossWirses> crossWirses) {
		...
		for (CrossWirses crossWirse : crossWirses) {
			...
			if(cPosition==-1){LOG.e(TAG, "服务器返回了不属于这这个表的行");continue;}
			//2.获得每一个问题的位置
			List<QuestionPortrait> queList = crossWirse.getQuestions();
			for (QuestionPortrait questionPortrait : queList) {
				int qPosition = getQPosition(questionPortrait,datas.get(cPosition));
				if(qPosition==-1){LOG.e(TAG, "服务器返回了不属于这一行的问题");continue;}
				
				int weight = datas.get(cPosition).getQuestions().get(qPosition).getWeightVal();
				//找出每一个答案的位置
				...
				int aPosition = getAPosition(answer, datas.get(cPosition).getQuestions().get(qPosition));
	...
	private int getQPosition(QuestionPortrait questionPortrait,CrossWirses crossWirse) {
		int qPosition = -1;
