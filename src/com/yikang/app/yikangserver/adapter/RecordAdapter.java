	...
	private void fillView(View v, int position, int type) {
		if (type == 0) {
			TextView tvTime = (TextView) v
					.findViewById(R.id.tv_item_record_time);
			// 由于服务器暂时返回的时间是错误的时间，故截取一段
			String name = data.get(position).getAssessmentName();
			if (!TextUtils.isEmpty(name)) {
				tvTime.setText(data.get(position).getAssessmentName()
						.substring(0, 10));
			}

			TextView tvCount = (TextView) v
					.findViewById(R.id.tv_item_record_count);
			tvCount.setText("第" + (data.size() - position) + "次评估");
		} else {
			// ImageView ibtnNew = (ImageView)
			// v.findViewById(R.id.ibtn_item_record_new);
