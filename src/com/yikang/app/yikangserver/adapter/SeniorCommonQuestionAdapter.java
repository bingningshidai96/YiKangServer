...
public class SeniorCommonQuestionAdapter extends SingleChoiseAdapter{
	...
	public SeniorCommonQuestionAdapter(Context context, List<Question> datas,int answerItemId) {
		super(context, datas, R.layout.item_question_senior_common_question, answerItemId);
	}
	...
	protected void fillContent(ViewHolder holder, Question question) {
		...
		TextView tvDecreption = holder.getView(R.id.tv_item_question_topic_detial);
		...
		for (int index = 0; index < rgAnswers.getChildCount(); index++) {
			...
			if(answerMap.get(position)!=null&&answerMap.get(position)==index){
