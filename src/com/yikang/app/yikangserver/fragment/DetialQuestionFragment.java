...
public class DetialQuestionFragment extends Fragment implements OnClickListener {
	 ...
	 * @return
	 */
	public static DetialQuestionFragment newInstance(CrossWirses cWirses, TableType tableType) {
	...
	protected void getData() {
		...
		loadingDialog.show();
		String json = QuestionData.readFileFromAssert(getActivity(), "daily_nursing_detials.txt");
		String data = null;
		try {
			JSONArray jarray = new JSONArray(json);
			for(int i=0;i<jarray.length();i++){
				JSONObject jo = jarray.getJSONObject(i);
				if(jo.getInt("questionCrosswiseId")== cWirses.getQuestionCrosswiseId()){
    	...
    	if(isRecord||(submitSet.contains(EvalutionState.currTableId)
    			&&crossSet.contains(cWirses.getQuestionCrosswiseId()))){
