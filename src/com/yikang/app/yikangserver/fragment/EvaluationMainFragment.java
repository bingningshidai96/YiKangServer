...
public class EvaluationMainFragment extends Fragment implements com.yikang.app.yikangserver.adapter.TotalPointChangedListener,OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
        	tableType = (TableType) getArguments().getSerializable(EXTRA_TABLE_TYPR);
    ...
    private void initAdapter(TableType type) {
		...
		case social_parti: //社会参与评估
			...
			((SinglePointChoiseAdapter)adapter).setTotalPointChangedListener(this);
			break;
    	...
    	BuisNetUtils.requestStr(url, param, new BuisNetUtils.ResponceCallBack() {
			...
			public void onSuccess(ResponseContent content) {
				...
				try {
					...
					List<Question> list = JSON.parseArray(data, Question.class);
    	...
    	HttpUtils.requestPost(UrlConstants.getQustrionSubmitUrl(tableType), 
    		...
    		public void postResult(String result) {
    			...
    			try {
    				...
    				if(ResponseContent.STATUS_OK.equals(resposeContent.getStatus())){
    					AppContext.showToast("提交成功");
    					//通知主fragment提交成功
    					listener.onInterAction(EvaInterActctionListnter.WHAT_SUBMIT, 
    							surveyId);
    				}else{
    					String failTip = AppContext.getStrRes(R.string.submit_fail_redo);
