...
public class SeniorInfoActivity extends BaseActivity implements OnClickListener,AMapLocationListener,OnDropDownItemClickListener{
	private static final String TAG = "SeniorInfoActivity";
	private Button btSubmit;
	private TextSpinner edpSex, edpIdentType, edpRace, edpFaith,
					edpLiveWith,edpPayType, edpIncomeSour, 
					edpRoomOrientation,edpOutWindow,edpCity,edpArea;
	...
	public static final int MAX_LOCATTIMES = 10; //最大定位次数，如果还不能获得结果，就认为定位失败
	...
	private void addEvaluationBag(final int seniorId){
		RequestParam param = new RequestParam();
		Map<String, Object> paramData = new HashMap<String, Object>();
		paramData.put("userId", AppContext.getAppContext().getUserId());
	...
	public void onClick(View v) {
		...
		if(id==R.id.bt_senior_info_submit){
			...
			HttpUtils.requestPost(UrlConstants.URL_SENOIR_INFO_SUBMIT, params, new HttpUtils.ResultCallBack() {
				@Override
				public void postResult(String result) {
					...
					try {
						reContent = ResponseContent.toResposeContent(result);
						if(ResponseContent.STATUS_OK.equals(reContent.getStatus())){
							Toast.makeText(getApplicationContext(), "提交成功", Toast.LENGTH_SHORT).show();
							LOG.i(TAG, reContent.getData());
							JSONObject jo = new JSONObject(reContent.getData());
							int seniorId = jo.getInt("seniorId");
							LOG.i(TAG, "获得seniorId:"+seniorId);
							notifyAddSenior();
							addEvaluationBag(seniorId);
						}else{
							Toast.makeText(getApplicationContext(), "提交失败，请重新尝试", Toast.LENGTH_SHORT).show();
						}
					} catch (Exception e) {
						e.printStackTrace();
	...
	private void notifyAddSenior() {
		sendBroadcast(new Intent(ConstantData.ACTION_BROADCAST_ADD_SENOIR));
	}
	/**
	 * 初始化各种输入框
	 */
	private void initEditors() {
		edpSex.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.sexMap)));
		edpIdentType.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.identTypeMap)));
		edpRace.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.raceMap)));
		edpLiveWith.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.livWithMap)));
		edpFaith.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.faithMap)));
		edpIncomeSour.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.inCometSourceMap)));
		edpPayType.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.payMentMap)));
		edpRoomOrientation.setAdapter(new PopListAdapter(this, MyData.getItems(MyData.roomOritentationMap)));
		edpOutWindow.setAdapter(new PopListAdapter(this,MyData.getItems(MyData.outWindowMap)));
	...
	private HashMap<String, Object> genrateMapFromInput(){
		...
		if(!isNoEmpty){ //如果包含空的数据
			Toast.makeText(this, "抱歉，您还有数据为空,请检查输入的数据", Toast.LENGTH_SHORT).show();
			return null;
		}
		...
		addToParamMap("floor", !TextUtils.isEmpty(floor)?Integer.parseInt(floor):null, map);
	...
	public void addToParamMap(String key,String srcValue,SparseArray<String> srcMap,HashMap<String, Object> desmap){
		...
		if(code==-100){ //-100代表找不到
			LOG.w(TAG, "addToMap 方法传入参数srcValue+'"+srcValue+"'在对应的spaseArray中找不到数据");
			return ;
		}
		addToParamMap(key, code, desmap);
	}
	
	public void addToParamMap(String key,Object value,HashMap<String, Object> desmap){
	...
	public void onLocationChanged(AMapLocation location) {
		...
		if(loacatTimes>MAX_LOCATTIMES){
			if(latitude==-1&&longitude==-1){
				Toast.makeText(this, "抱歉，定位失败。请开启wifi或GPS", Toast.LENGTH_LONG).show();
			}
			endLocate();//结束定位
		}
